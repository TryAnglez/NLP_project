...
class ChatGPT2:
    def __init__(self, temperature=1.0, model_name='gpt2', device='cpu'):
    ...
    def __call__(self, prompt, **kwargs):
        ...
        with torch.no_grad():
            logits = self.model.generate(inputs, **kwargs)[0]
            logits /= self.temperature
        return self.tokenizer.decode(logits.squeeze())[len(prompt):].strip()
...
##### streamlit에서 캐싱을 사용해 대용량 데이터셋이나 복잡한 계산을 수행할 때 웹 앱의 성능을 크게 향상시킬 수 있다 #####
#  streamlit에선 앱에서 상호작용이 발생할 때마다 전체 스크립트가 처음부터 다시 실행된다 -> 프로그래밍 모델을 간단하게 만들지만 비효율성을 초래할 수 있다
...
def load_chain(_client):    
    ...
    # )
    chain = ConversationalRetrievalChain.from_llm(
            llm=ChatGPT2(temperature=1.0, model_name='gpt2'),
            retriever=vectorstore.as_retriever()
    )
    return chain
